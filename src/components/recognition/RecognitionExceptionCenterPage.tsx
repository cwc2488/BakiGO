...
export function RecognitionExceptionCenterPage({ eventId }: { eventId: string }) {
  ...
  return (
    <PageShell
      title="例外處理"
      subtitle="只處理截止後仍未解決、或系統無法可靠判斷的資料"
    ...
    >
      ...
      {!loading && items.length === 0 && (
        <BrandCard variant="bordered">
          <p className="text-[1.0625rem] font-semibold text-[#1d1d1f]">沒有待處理例外</p>
      ...
      {items.map((item) => (
        <BrandCard key={item.entryId} variant="bordered">
          ...
          <p className="mt-1 text-[0.8125rem] text-[#86868b]">投稿者 {item.submitterName} · {item.validationStatus}</p>
          <ul className="mt-3 flex flex-col gap-1 text-[0.875rem] text-[#ff375f]">
