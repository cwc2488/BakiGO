...
function NodeCard({
  ...
  return (
    <div className="flex flex-col items-center">
      <button
        className={`relative w-[9.5rem] rounded-2xl border px-3 py-3 text-left transition-all duration-200 sm:w-[10.5rem] ${
          isSelected
            ? "border-[var(--brand-primary)] bg-[var(--brand-primary-light)] shadow-[0_4px_20px_rgba(119,181,57,0.15)]"
      ...
      >
        ...
        />
        <p className="pr-4 text-[0.9375rem] font-semibold leading-snug text-[#1d1d1f]">
          {member.name}
        </p>
        <p className="mt-1 text-[0.75rem] text-[#86868b]">{member.qualificationLabel}</p>
        <p className="mt-2 text-[0.8125rem] font-medium text-[#636366]">
          {member.monthlyVp} VP
        </p>
        {hasChildren ? (
          <p className="mt-1 text-[0.75rem] text-[#aeaeb2]">
            第一代 {member.directDownlineCount} 位
          </p>
        ) : null}
      </button>
      ...
      {hasChildren ? (
        <button
          ...
          className="mt-2 flex h-6 w-6 items-center justify-center rounded-full bg-[var(--brand-bg)] text-[0.875rem] font-semibold text-[#636366] transition-colors hover:bg-[var(--brand-border)]"
          onClick={(event) => {
  ...
  return (
    <>
      ...
      {hasChildren && isExpanded ? (
        <>
          <div aria-hidden className="h-5 w-px bg-[#d1d1d6]" />
          <div className="relative w-full overflow-x-auto pb-2">
            <ul className="relative inline-flex min-w-full items-start justify-center gap-6 px-4 pt-5">
              <div
                aria-hidden
                className="pointer-events-none absolute left-8 right-8 top-0 h-px bg-[#d1d1d6]"
              />
              {node.children.map((child) => (
                <li key={child.member.memberId} className="relative flex flex-col items-center">
                  <div
                    aria-hidden
                    className="absolute -top-5 left-1/2 h-5 w-px -translate-x-1/2 bg-[#d1d1d6]"
                  />
                  <TreeBranch
...
export function OrganizationTreeDiagram({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="overflow-x-auto rounded-[1.75rem] border border-[var(--brand-border)] bg-[var(--brand-primary-muted)] px-4 py-8 sm:px-6">
