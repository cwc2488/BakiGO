/**
 * P0.2 Preview latency benchmark — Scenarios A/B/C ×3.
 *
 * Usage:
 *   PREVIEW_BASE_URL=https://baki-egpi6t1gg-baki-go.vercel.app \
 *   npx vercel env run --environment=preview -- npx tsx scripts/run-p02-preview-latency-benchmark.ts
 *
 * Never prints secrets / PII / prompt text / photo content.
 * Writes .tmp-p02-preview-latency-benchmark.json
 */
...
async function claimJobForDay(input: {
  ...
  for (let attempt = 0; attempt < 6; attempt += 1) {
    const claimed = await claimCoachingGenerationJobs({
      limit: 5,
      lockedBy: `p02-bench-${randomBytes(4).toString("hex")}`,
    });
...
async function main() {
  ...
  const report = {
    type: "p02_preview_latency_benchmark",
  ...
  writeFileSync(".tmp-p02-preview-latency-benchmark.json", JSON.stringify(report, null, 2));
  console.log(
    `P02_BENCH:${JSON.stringify({
...
main().catch((error) => {
  console.error(
    `P02_BENCH:${JSON.stringify({ ok: false, error: error instanceof Error ? error.message : String(error) })}`,
  );
  process.exit(1);
});
