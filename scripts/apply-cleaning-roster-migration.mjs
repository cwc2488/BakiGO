/**
 * Apply CLEANING-ROSTER-01 migration 077 (idempotent, additive).
 *
 *   npx vercel env run -e production -- node scripts/apply-cleaning-roster-migration.mjs
 *   # or with env already present:
 *   node scripts/apply-cleaning-roster-migration.mjs
 */
...
async function main() {
  ...
  const sql = readFileSync("supabase/migrations/077_cleaning_roster_v1.sql", "utf8");
  ...
  if (accessToken) {
    ...
    if (!resp.ok) {
      console.log(JSON.stringify({ ok: false, via: "management_api", status: resp.status, body: text.slice(0, 2000) }));
      process.exit(1);
    }
  ...
  // Fallback probe: tables readable after SQL applied externally.
  const { error } = await supabase.from("cleaning_roster_meta").select("id").limit(1);
  if (error) {
    console.log(
      JSON.stringify({
        ok: false,
        ...
        error: error.message,
        hint: "Apply 077_cleaning_roster_v1.sql via Supabase SQL Editor or SUPABASE_ACCESS_TOKEN.",
      }),
    );
    process.exit(1);
  }
