-- CUSTOMER-DURABLE-DELETE-01
-- Soft-delete authority on customers.deleted_at.
-- Stale client upserts must not clear a set deleted_at (no restore in this task).
-- Related child rows stay intact (no hard DELETE / no FK cascade).

alter table public.customers
  add column if not exists deleted_at timestamptz null;

comment on column public.customers.deleted_at is
  'Soft-delete timestamp. NULL = active. Once set, client upserts must not clear it.';
...
create or replace function public.customers_preserve_deleted_at()
returns trigger
language plpgsql
as $$
begin
  -- Server deletion authority: once soft-deleted, deleted_at cannot be cleared
  -- by normal client update/upsert payloads (including omitted → null coercion).
...
drop trigger if exists customers_preserve_deleted_at on public.customers;
create trigger customers_preserve_deleted_at
  before update on public.customers
  for each row
  execute function public.customers_preserve_deleted_at();

-- Magic Link portal: hide soft-deleted customers (payload shape matches 051).
