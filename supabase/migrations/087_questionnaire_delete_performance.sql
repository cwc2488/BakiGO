    ...
    select
      count(*) filter (
        where fish_credited_at is not null
          and (fish_credited_at at time zone 'Asia/Taipei')::date = p_today
      ...
      count(*) filter (
        where fish_credited_at is not null
          and (fish_credited_at at time zone 'Asia/Taipei')::date >= p_week_start
          and (fish_credited_at at time zone 'Asia/Taipei')::date <= p_today
...
begin
  ...
  if v_lead.fish_credited_at is not null then
    ...
    insert into public.five_plus_five_reports (
      ...
      v_fish_date,
      0, 0, 0, 0, 0, 0,
      p_now, p_now, false, p_now, false, null
    )
    ...
    if found then
      ...
      set
        questionnaire_fish_pool_count = greatest(questionnaire_fish_pool_count - 1, 0),
        updated_at = p_now
      where id = v_row.id;
